...
class QtConan(ConanFile):
    ...
    def source(self):
        ...
        src_definition["headers"]={"X-JFrog-Art-Api":os.environ["ARTIFACTORY_API_KEY"]}
        tools.get(**src_definition)
    ...
    def build(self):
        ...
        if "arm" in str(self.settings.arch) and self.settings.os == "Macos":
            args += ["-device-option QMAKE_APPLE_DEVICE_ARCHS=arm64"]
